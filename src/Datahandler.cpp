...
void Datahandler::sendMqtt(){
    ...
    std::string topic;
    std::string payload;
    std::map<std::string, std::string> macAddressesToNames = {
        {"EB740996B645", "Balkon"},
        {"EE23032697E4", "Fenster"},
        {"DA59E068B395", "Wohnzimmer"}
    };
    ...
    network::mqtt::publishDiscovery(macAddressesToNames[macAddress]);
    ...
    stream << macAddressesToNames[macAddress] << "/state";
    topic=stream.str();
    ...
    stream << "\"tempRising\":" << measurement.getTempRising() << "}";

    payload=stream.str();

    Serial.print("Publishing measurements over MQTT for ");
