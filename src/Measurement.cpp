...
Measurement::Measurement(){
    ...
    lastMeasurements.fill(0.0); // Initialize all elements to 0.0
}
...
double Measurement::averageLastMeasurements() {
    double sum = std::accumulate(lastMeasurements.begin(), lastMeasurements.end(), 0.0);
    Serial.println(sum);
    return sum / 10.0;
}

bool Measurement::isMeasurementHigher(){
    return temperature > averageLastMeasurements();
}
...
void Measurement::build(string dataIn){
    ...
    // Update lastMeasurements
    lastMeasurements[currentIndex] = temperature;
    currentIndex = (currentIndex + 1) % 10;  // Cycle back to 0 after reaching 10

    tempRising = isMeasurementHigher();
}
...
std::string Measurement::toStr(Measurement::measurementFormatT measurementFormat){
    ...
    if(measurementFormat==Measurement::longColumnName){
        ...
        sTempRising  << "tempRising="  << tempRising;
    }else{
...
bool Measurement::getTempRising(){
    return tempRising;
}